...
public interface TaskRepository extends JpaRepository<Task, Long> {
    ...
    @Transactional
    @Modifying
    @Query(value = "update task set column_position= column_position-1 where project_id=?1 and status_id=(select status_id from (SELECT * FROM task) as status where id=?2) and column_position > (select column_position from (SELECT * FROM task) as position where id=?2) and column_position <= ?3 and id != ?2", nativeQuery = true)
    void updateColumnPositionWhenChangeTaskPositionDown(Long projectId, Long task_id, int columnPosition);

    @Transactional
    @Modifying
    @Query(value = "update task set column_position= column_position+1 where project_id=?1 and status_id=(select status_id from (SELECT * FROM task) as status where id=?2) and column_position < (select column_position from (SELECT * FROM task) as position where id=?2) and column_position >= ?3 and id != ?2", nativeQuery = true)
    void updateColumnPositionWhenChangeTaskPositionUp(Long projectId, Long task_id, int columnPosition);
